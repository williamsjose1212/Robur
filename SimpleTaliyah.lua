...
function Taliyah.Logic.Combo()
  ...
  if MenuValueW and Taliyah.W:IsReady() and Player.Mana > wMana + eMana then
    for k, enemy in ipairs(Utils.GetTargets(Taliyah.W)) do
      ...
      if wPred ~= nil and Utils.IsValidTarget(enemy) and wPred.HitChanceEnum >= HitChanceEnum.Medium and (Taliyah.E:IsReady() or Utils.GetDamage(enemy) > hpPred or Player.Health - incomingDamage < enemies * Player.Level * 15 or eIsOn or Taliyah.E:GetLevel() == 0 or wHitCount > 1 )  and Player:Distance(enemy.Position) > 420 then
      ...
      elseif wPred ~= nil and Utils.IsValidTarget(enemy) and wPred.HitChanceEnum >= HitChanceEnum.Medium and (Taliyah.E:IsReady() or Utils.GetDamage(enemy) > hpPred or Player.Health - incomingDamage < enemies * Player.Level * 15 or eIsOn or Taliyah.E:GetLevel() == 0 or wHitCount > 1) and Player:Distance(enemy.Position) < 420 then
        if wPred.TargetPosition:Distance(wPred.CastPosition) <= 400 then
