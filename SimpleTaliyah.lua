...
function Taliyah.Logic.Combo()
  ...
  if MenuValueW and Taliyah.W:IsReady() and Player.Mana > wMana + eMana then
    for k, enemy in ipairs(Utils.GetTargets(Taliyah.W)) do
      ...
      if wPred ~= nil and wPred.HitChanceEnum >= HitChanceEnum.VeryHigh and Utils.IsValidTarget(enemy) and (Taliyah.E:IsReady() or Utils.GetDamage(enemy) > hpPred or Player.Health - incomingDamage < enemies * Player.Level * 15 or eOnGround or Taliyah.E:GetLevel() == 0 or wHitCount > 1 )  and Player:Distance(enemy.Position) > 420  and wPred.TargetPosition:Distance(wPred.CastPosition) <= 200 then
        if Input.Cast(SpellSlots.W,Player.Position,wPred.CastPosition) then return true end
      elseif wPred ~= nil and wPred.HitChanceEnum >= HitChanceEnum.VeryHigh and Utils.IsValidTarget(enemy) and (Taliyah.E:IsReady() or Utils.GetDamage(enemy) > hpPred or Player.Health - incomingDamage < enemies * Player.Level * 15 or eOnGround or Taliyah.E:GetLevel() == 0 or wHitCount > 1) and Player:Distance(enemy.Position) < 420 and wPred.TargetPosition:Distance(wPred.CastPosition) <= 200 then
